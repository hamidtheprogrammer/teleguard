...
async function fillDB() {
  ...
  for (let i = 0; i < patients.length; i++) {
    const hashedPassword = await bcrypt.hash(patients[i].password, salt);

    const newUser = {
      username: encrypt(patients[i].username),
      email: encrypt(patients[i].email),
      password: hashedPassword,
      phone: encrypt(patients[i].phone),
      role: patients[i].role as Role,
      verified: true,
    };
